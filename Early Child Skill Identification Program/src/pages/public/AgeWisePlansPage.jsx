...
export const AgeWisePlansPage = ({ setActivePage, setActivePortal }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
          ...
          {ageGroupPlansData.map((plan, idx) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-extrabold transition-all ${
                activeTab === idx ? `${activeConfig.buttonPrimary}` : 'bg-white text-slate-700 border border-slate-200'
              }`}
            >
              {plan.ageGroup}
            </button>
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl space-y-6 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {selectedPlan.programs.map((p) => (
            <div key={p.id} className="p-6 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 text-xs">
