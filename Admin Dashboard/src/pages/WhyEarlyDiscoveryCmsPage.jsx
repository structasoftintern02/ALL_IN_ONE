...
import { 
  Target, Save, Eye, EyeOff, Plus, Trash2, RefreshCw, Layers, Brain, Sparkles, Zap, HeartHandshake, ShieldCheck, Award
} from 'lucide-react';
import { useApp } from '../context/AppContext';

const API_BASE = 'http://localhost:5000/api';

const defaultWhyEarlyCms = {
  badge: "💡 Why Early Skill Identification Matters",
  title: "Why Discovering Talent Early Matters",
  highlightText: "Early Matters",
  ...
  buttonText: "Read Our Scientific Methodology →",
  cards: [
    {
      id: 'we-1',
      iconName: 'Brain',
...
export const WhyEarlyDiscoveryCmsPage = () => {
  const { showToast } = useApp();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [cms, setCms] = useState(defaultWhyEarlyCms);
  ...
  const fetchCmsData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (data && data.whyEarlyDiscoveryCms) {
          setCms({
            ...defaultWhyEarlyCms,
            ...data.whyEarlyDiscoveryCms,
            visibility: { ...defaultWhyEarlyCms.visibility, ...(data.whyEarlyDiscoveryCms.visibility || {}) }
          });
        }
      }
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.ok) {
        showToast('Why Early Discovery Section updated & published live!', 'success');
      } else {
  ...
  const handleAddCard = (e) => {
    ...
    const cardToAdd = {
      ...
      desc: newCard.desc.trim() || 'Custom early discovery benefit description',
    ...
    showToast('New Why Early Discovery Card added!', 'info');
  };
  ...
  if (loading) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: 'var(--slate-500)' }}>
        ...
        <p style={{ marginTop: 12, fontWeight: 700 }}>Loading Why Early Discovery CMS...</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, paddingBottom: 40 }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 900, margin: 0 }}>Why Early Discovery Section CMS</h1>
          </div>
          <p style={{ fontSize: 13, color: '#C7D2FE', marginTop: 4, marginBottom: 0, maxWidth: 650 }}>
            Customize the badge, main headline, subtitle, 6 early discovery cards (emojis & text), and CTA button that display live on the Home Page.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, alignItems: 'start' }}>
          ...
          <div className="card">
            <div className="card-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h3>
                ...
                <span>2. Early Discovery Cards ({cms.cards?.length || 0} Cards)</span>
              </h3>
            </div>
        ...
        {/* Right Column: Live Real-Time Website Preview Box (Matching User Screenshot 2) */}
            ...
            <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 16, textAlign: 'center' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 6 }}>
                {(cms.cards || []).slice(0, 6).map((c, i) => (
                  ...
                  >
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: 'linear-gradient(135deg, #8B5CF6, #EC4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: 'white' }}>
                      <Brain size={14} />
                    </div>
