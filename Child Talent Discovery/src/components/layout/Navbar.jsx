...
import {
  Sparkles, Sun, Moon, Menu, X, ChevronDown, ArrowRight, Palette,
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-18 py-2 gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 sm:gap-3 group text-left flex-shrink-0 select-none"
            id="nav-brand-logo"
          >
            <div className={`w-9 h-9 sm:w-11 sm:h-11 ${activeConfig.cardRadius} bg-gradient-to-tr ${activeConfig.primaryGradient} flex items-center justify-center text-white shadow-md group-hover:scale-105 transition-transform`}>
              <Sparkles className="w-5 h-5 sm:w-6 sm:h-6 animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-1">
                <span className="text-base sm:text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                  Talent<span className={`bg-gradient-to-r ${activeConfig.gradientText} bg-clip-text text-transparent`}>Discovery</span>
                </span>
                <span className={`text-[9px] sm:text-[10px] uppercase tracking-widest font-extrabold px-1.5 py-0.5 rounded-full ${activeConfig.accentBadge}`}>
                  PRO
                </span>
              </div>
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {mainPages.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 rounded-xl text-sm font-semibold transition-all whitespace-nowrap relative ${
                    isActive
                ...
                >
                  {link.label}
                  {isActive && (
                    <span className="absolute bottom-0.5 left-3 right-3 h-0.5 bg-gradient-to-r from-rose-500 to-purple-500 rounded-full" />
                  )}
                </button>
              );
            })}
            ...
            <div className="relative">
              <button
                ...
                className={`px-3 py-2 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100/70 dark:hover:bg-slate-800/60 flex items-center gap-1 transition-all whitespace-nowrap ${
          ...
          {/* Right Actions: Theme Switcher & Dark Mode Toggle */}
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            ...
            <div className="relative">
              <button
                ...
                className={`px-2.5 sm:px-3 py-1.5 sm:py-2 ${activeConfig.cardRadius} bg-gradient-to-r ${activeConfig.primaryGradient} text-white text-xs font-extrabold flex items-center gap-1.5 shadow-md hover:opacity-95 transition-all whitespace-nowrap`}
              ...
              <AnimatePresence>
                {themeDropdownOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8, scale: 0.95 }}
                    transition={{ duration: 0.15 }}
                    className={`absolute right-0 mt-2 w-64 bg-white dark:bg-slate-800 ${activeConfig.cardRadius} shadow-2xl border border-slate-200 dark:border-slate-700 py-2 z-50`}
                  >
                    <div className="px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-700 mb-1">
                      Choose UI/UX Theme Variation
                    </div>
                    {Object.values(themeConfigs).map((t) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-xs font-bold flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors ${
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div>{t.name}</div>
                            <div className="text-[10px] font-normal text-slate-500 dark:text-slate-400">{t.tagline.slice(0, 30)}...</div>
                          </div>
                        </div>
            ...
            >
              {isDark ? <Sun className="w-5 h-5 text-amber-400 animate-spin-slow" /> : <Moon className="w-5 h-5 text-purple-600" />}
            </button>
            ...
            <motion.button
              ...
              className={`hidden md:flex items-center gap-2 px-4 py-2.5 ${activeConfig.cardRadius} text-xs font-extrabold transition-all whitespace-nowrap ${activeConfig.buttonPrimary}`}
            >
