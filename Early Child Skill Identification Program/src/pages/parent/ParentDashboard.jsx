      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 overflow-y-auto w-full min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 text-xs">
          ...
          <div className={`p-5 bg-white ${activeConfig.cardRadius} border border-slate-200 shadow-xs space-y-2`}>
            ...
            <div className="text-3xl font-extrabold text-pink-600">{child.overallProgressScore}%</div>
          ...
          <div className={`p-5 bg-white ${activeConfig.cardRadius} border border-slate-200 shadow-xs space-y-2`}>
            ...
            <div className="text-3xl font-extrabold text-purple-600">{child.skillScores.cognitiveLogic}%</div>
          ...
          <div className={`p-5 bg-white ${activeConfig.cardRadius} border border-slate-200 shadow-xs space-y-2`}>
            ...
            <div className="text-3xl font-extrabold text-pink-500">{child.skillScores.creativeArtistic}%</div>
          ...
          <div className={`p-5 bg-white ${activeConfig.cardRadius} border border-slate-200 shadow-xs space-y-2`}>
            ...
            <div className="text-3xl font-extrabold text-amber-600">{child.skillScores.fineMotor}%</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          ...
          <div className={`lg:col-span-6 bg-white ${activeConfig.cardRadius} border border-slate-200 p-5 sm:p-6 space-y-4 shadow-sm text-xs`}>
            <h3 className="font-bold text-slate-900 text-sm">Upcoming Skill Sessions</h3>
            <div className="space-y-3">
              {child.upcomingSessions.map((s) => (
                <div key={s.id} className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
          ...
          <div className={`lg:col-span-6 bg-white ${activeConfig.cardRadius} border border-slate-200 p-5 sm:p-6 space-y-4 shadow-sm text-xs`}>
            ...
            <div className="space-y-2">
              {child.recommendations.map((rec, idx) => (
                <div key={idx} className="p-3 bg-pink-50/60 rounded-xl border border-pink-100 text-slate-800 flex items-start gap-2">
