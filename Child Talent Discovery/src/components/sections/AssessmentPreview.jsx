...
import { sampleReportData } from '../../data/talentData';
    ...
    <section className="py-20 lg:py-28 bg-white dark:bg-slate-900" id="assessment-preview">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <SectionHeader
          ...
          title={<>Explore a Real <span className={`bg-gradient-to-r ${activeConfig.gradientText} bg-clip-text text-transparent`}>Talent Discovery Report</span></>}
        ...
        <div className={`p-6 sm:p-10 ${activeConfig.cardRadius} bg-slate-900 text-white shadow-2xl border border-purple-900/50 relative overflow-hidden space-y-8`}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-xl font-extrabold text-white">{sampleReportData.childName}</h3>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                    Verified Profile
                  </span>
                </div>
                <p className="text-xs text-slate-400 font-semibold">
                  Age: {sampleReportData.age} • Assessed: {sampleReportData.assessmentDate}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="px-4 py-2 rounded-xl bg-purple-950/60 border border-purple-800/60 text-right">
                ...
                <div className="text-xl font-extrabold text-amber-400">{sampleReportData.overallScore} / 100</div>
              </div>
              <button
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-900/60 via-slate-800 to-indigo-900/60 border border-purple-700/40 flex items-center gap-4">
            ...
            <div>
              <div className="text-xs font-extrabold text-purple-300 uppercase tracking-widest">Identified Talent Archetype</div>
              <div className="text-base sm:text-lg font-extrabold text-white">{sampleReportData.archetype}</div>
              <p className="text-xs text-slate-300 mt-0.5 leading-relaxed">{sampleReportData.summary}</p>
            </div>
          </div>
            ...
            {activeTab === 'overview' && (
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {sampleReportData.skills.map((skill, i) => (
              ...
              >
                ...
                <div className="p-5 rounded-2xl bg-emerald-950/40 border border-emerald-800/60 space-y-3">
                  ...
                  <ul className="space-y-2">
                    {sampleReportData.strengths.map((str, idx) => (
                ...
                <div className="p-5 rounded-2xl bg-amber-950/40 border border-amber-800/60 space-y-3">
                  ...
                  <ul className="space-y-2">
                    {sampleReportData.growthAreas.map((grow, idx) => (
            ...
            {activeTab === 'learning-path' && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {sampleReportData.recommendedActivities.map((act, idx) => (
          ...
          <div className="text-center pt-4 border-t border-slate-800 text-xs text-slate-400">
            🔒 All assessments are 100% private, parent-guided, and based on observational play metrics.
          </div>
        ...
        <div className="text-center">
          ...
          >
            Get a Report Like This for Your Child →
          </button>
        </div>
