    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StaggerItem direction="scale">
          <motion.div whileHover={{ y: -4 }} className={`p-6 bg-white ${activeConfig.cardRadius} border border-slate-200 shadow-sm hover:shadow-md transition-all space-y-2`}>
      ...
      <ScrollReveal direction="up" amount={0.15} className={`bg-white ${activeConfig.cardRadius} ${activeConfig.cardBorder} p-6 sm:p-8 space-y-6 shadow-sm`}>
        ...
        <StaggerContainer className="space-y-6">
          {tranches.map((item, idx) => (
            <StaggerItem key={idx} direction="up">
              <motion.div whileHover={{ x: 4 }} className="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 transition-all">
                ...
                <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                  <motion.div 
                    initial={{ width: '0%' }}
                    animate={{ width: item.status === 'Released' ? '100%' : item.status === 'Pending Approval' ? '50%' : '0%' }}
                    transition={{ duration: 1 }}
                    className={`h-full rounded-full ${
                      item.status === 'Released' ? 'bg-emerald-500' : item.status === 'Pending Approval' ? 'bg-amber-500' : 'bg-slate-300'
                    }`} 
                  />
                </div>
              </motion.div>
