      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 overflow-y-auto w-full min-w-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <motion.button
                  ...
                  className="w-full sm:w-auto px-4 py-2.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-extrabold text-xs rounded-xl transition-all shadow-md"
                >
                  Upgrade Plan
                </motion.button>
                ...
                <motion.button
                  ...
                  className="w-full sm:w-auto px-4 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 text-white font-semibold text-xs rounded-xl backdrop-blur-xs transition-all"
                >
                  Manage Profile
                </motion.button>
              </div>
        ...
        <StaggerContainer className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <StaggerItem direction="scale">
            <motion.div whileHover={{ y: -4 }} className={`p-5 ${activeConfig.cardBg} ${activeConfig.cardRadius} ${activeConfig.cardBorder} space-y-2 glassmorphism-card shadow-sm hover:shadow-md transition-all`}>
              ...
              <div className={`text-2xl font-extrabold ${activeConfig.isDark ? 'text-white' : 'text-slate-900'}`}>
                <AnimatedCounter value={user.stats.totalSent} />
              </div>
          ...
          <StaggerItem direction="scale">
            <motion.div whileHover={{ y: -4 }} className={`p-5 ${activeConfig.cardBg} ${activeConfig.cardRadius} ${activeConfig.cardBorder} space-y-2 glassmorphism-card ai-glow-border relative overflow-hidden shadow-sm hover:shadow-md transition-all`}>
              <div className="flex items-center justify-between text-slate-400">
                <span className="text-[11px] font-bold uppercase tracking-wider text-cyan-400">GPT-4o Auto-Replies</span>
                <Bot className="w-4 h-4 text-cyan-400 animate-pulse" />
              </div>
              <div className={`text-2xl font-extrabold ${activeConfig.isDark ? 'text-white' : 'text-slate-900'}`}>
                <AnimatedCounter value={user.stats.aiAutoReplied} />
              </div>
          ...
          <StaggerItem direction="scale">
            <motion.div whileHover={{ y: -4 }} className={`p-5 ${activeConfig.cardBg} ${activeConfig.cardRadius} ${activeConfig.cardBorder} space-y-2 glassmorphism-card shadow-sm hover:shadow-md transition-all`}>
              ...
              <div className={`text-2xl font-extrabold ${activeConfig.isDark ? 'text-white' : 'text-slate-900'}`}>
                <AnimatedCounter value={user.subscription.contactsCount} />
              </div>
        ...
        <ScrollReveal direction="up" amount={0.2} className={`p-4 ${activeConfig.cardBg} ${activeConfig.cardRadius} ${activeConfig.cardBorder} flex flex-wrap items-center justify-between gap-3 glassmorphism-card`}>
          <span className="text-xs font-bold uppercase tracking-wider text-slate-400 px-2">
            Quick Action Controls
          </span>
          <div className="flex flex-wrap items-center gap-2">
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs flex items-center gap-1.5 shadow-sm transition-all">
              <Send className="w-3.5 h-3.5" />
              <span>New Broadcast Campaign</span>
            </motion.button>
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs flex items-center gap-1.5 shadow-sm transition-all">
              <Bot className="w-3.5 h-3.5" />
              <span>Build AI Flow</span>
            </motion.button>
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs flex items-center gap-1.5 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          ...
          <ScrollReveal direction="right" className={`lg:col-span-7 ${activeConfig.cardBg} ${activeConfig.cardRadius} ${activeConfig.cardBorder} p-5 sm:p-6 space-y-4 glassmorphism-card`}>
            ...
            <div className="space-y-3">
              {user.recentActivities.map((act) => (
                <motion.div key={act.id} whileHover={{ x: 4 }} className="p-3.5 bg-slate-50 dark:bg-gray-800/60 rounded-xl border border-slate-200 dark:border-gray-700/60 flex items-center justify-between text-xs transition-all hover:border-emerald-500/40">
          ...
          <ScrollReveal direction="left" className={`lg:col-span-5 ${activeConfig.cardBg} ${activeConfig.cardRadius} ${activeConfig.cardBorder} p-5 sm:p-6 space-y-4 glassmorphism-card`}>
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-gray-800 pb-3">
              <h3 className={`font-bold text-sm ${activeConfig.isDark ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div className="space-y-2.5">
              {user.notifications.map((n) => (
                <div key={n.id} className="p-3 bg-slate-50 dark:bg-gray-800/60 rounded-xl border border-slate-200 dark:border-gray-700 text-xs flex items-start gap-2.5">
