      ...
      <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 overflow-y-auto w-full min-w-0">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-full sm:w-auto px-4 py-2.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-extrabold text-xs rounded-xl shadow-md"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          ...
          <div className={`p-5 bg-white ${activeConfig.cardRadius} border border-slate-200 shadow-xs space-y-2`}>
            ...
            <div className="text-2xl font-extrabold text-slate-900">{candidate.profileCompletionScore}%</div>
            <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
              <div className="h-full bg-emerald-500 rounded-full w-11/12" />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          ...
          <div className={`lg:col-span-7 bg-white ${activeConfig.cardRadius} border border-slate-200 p-5 sm:p-6 space-y-4 shadow-sm`}>
            ...
            <div className="space-y-3">
              {candidate.appliedJobs.map((app) => (
                <div key={app.id} className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs">
                  <div>
          ...
          {/* Recommended Jobs */}
          <div className={`lg:col-span-5 bg-white ${activeConfig.cardRadius} border border-slate-200 p-5 sm:p-6 space-y-4 shadow-sm`}>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <div className="space-y-3">
              {jobListingsData.slice(0, 2).map((job) => (
                <div key={job.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2 text-xs">
